...
import { formatTime, phoneCountryCodes } from '@/lib/config/constants';
import { ArrowLeftIcon, ArrowRightIcon } from 'lucide-react';
import { cn, findEnumKey } from '@/lib/utils/utils';
...
export const AuthForm = ({ children }: { children: React.ReactNode }) => {
    ...
    const onSubmit = useCallback(async (formdata: Record<FieldType, string>) => {
        ...
        try {
            ...
            console.log('data', response,'form', response?.data, response.data?.form);
            if (response.success && response.data) {
                const { form, sessionId, user } = response.data;
                if (form && form.nextStep && form.inAuthSessionID) {
                    ...
                    setHintValue({ emailorPhone: screens.fields[0].hintValue!, firstname:  screens.fields[0].profileHint?.firstname! });
                }
            }
